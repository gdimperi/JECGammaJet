...
def createProcess(runOnMC, runCHS, correctMETWithT1, processCaloJets, processCA8Jets, globalTag):
  ...
  from PhysicsTools.PatAlgos.patTemplate2_cfg import process
  ...
  # chiara: forse qui analogo, ma non so se CHS si usi o meno per questa collezione
  ...
  if runCHS:
    ...
    ## Setup quark gluon tagger
    #process.load('QuarkGluonTagger.EightTeV.QGTagger_RecoJets_cff')
    #cloneProcessingSnippet(process, process.QuarkGluonTagger, PFlowCA8)
    #getattr(process, "QGTaggerPFlowCA8").srcJets = cms.InputTag("selectedPatJetsPFlowCA8")
    #getattr(process, "QGTaggerPFlowCA8").isPatJet = cms.untracked.bool(True)
    #getattr(process, "QGTaggerPFlowCA8").useCHS = cms.untracked.bool(usePFNoPU)

    ## Remove the processing of primary vertices, as it's already what we do here
    #getattr(process, 'QGTaggerPFlowCA8').srcPV = cms.InputTag('goodOfflinePrimaryVertices')
    #getattr(process, 'QuarkGluonTaggerPFlowCA8').remove(getattr(process, 'goodOfflinePrimaryVerticesQGPFlowCA8'))



    #setattr(process, 'QGTaggerPFlowCA8', process.QGTagger.clone())
    #getattr(process, "QGTaggerPFlowCA8").srcJets = cms.InputTag("selectedPatJetsPFlowCA8" )
    #getattr(process, "QGTaggerPFlowCA8").isPatJet = cms.untracked.bool(True)
    #getattr(process, "QGTaggerPFlowCA8").useCHS = cms.untracked.bool(False)
      
    #process.QuarkGluonTagger.replace(process.QGTagger, getattr(process, 'QGTaggerPFlowCA8'))

    #if runCHS:
    #  chsP = p + "chs"
    
    #  cloneProcessingSnippet(process, process.QuarkGluonTagger, "PFlowCA8chs")
    #  setattr(process, 'QGTaggerPFlowCA8chs', process.QGTagger.clone())
    #  getattr(process, "QGTaggerPFlowCA8chs").srcJets = cms.InputTag("selectedPatJetsPFlowCA8chs" )
    #  getattr(process, "QGTaggerPFlowCA8chs").isPatJet = cms.untracked.bool(True)
    #  getattr(process, "QGTaggerPFlowCA8chs").useCHS = cms.untracked.bool(True)
    
    #  process.QuarkGluonTagger.replace(getattr(process, 'QGTaggerPFlowCA8'), getattr(process, 'QGTaggerPFlowCA8') + getattr(process, 'QGTaggerPFlowCA8chs'))
